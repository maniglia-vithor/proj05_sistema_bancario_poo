...
class ContaBancaria:
    ...
    def __init__(self, numero_conta, saldo_inicial=0, extrato_inicial=None):
        ...
        self._numero_conta = numero_conta  # Corrigido para self._numero_conta
        self.extrato = extrato_inicial if extrato_inicial is not None else []

    def consultar_saldo(self):
        """Exibe o saldo atual da conta."""
        print(f"Seu saldo atual é: R$ {self.saldo:.2f}")  # Adicionado parêntese faltante
    ...
    def depositar(self, valor):
        ...
        try:
            ...
            if valor_deposito <= 0:
                print("\nValor de depósito inválido. Digite um número positivo.")  # Corrigido \olor para \nValor
                return
            ...
            print(f"Seu novo saldo é: R$ {self.saldo:.2f}")  # Added missing parenthesis
        except ValueError:
...
def exibir_extrato(self):
    ...
    if not self.extrato:  # Verifica se o extrato está vazio
        ...
        print("\n--- Extrato Bancário ---")  # Added extra '-' for consistency
        for transacao in self.extrato:  # Itera sobre a lista extrato e exibe cada transação formatada
            data_hora = transacao["data_hora"]
            tipo = transacao["tipo"]
            valor = transacao["valor"]
            print(f"{data_hora} - {tipo}: R$ {valor:.2f}")
        print(f"\nSaldo atual: R$ {self.saldo:.2f}")  # Added newline before saldo
...
def transferir(self, conta_destino, valor):
    ...
    try:
        ...
        if not isinstance(conta_destino, ContaBancaria):  # Verifica se a conta de destino é válida
            print("Erro: A conta de destino não é uma instância de ContaBancaria.")
            return
            
        if valor_transferencia <= self.saldo:  # Verifica se há saldo suficiente
            self.saldo -= valor_transferencia  # Subtrai o valor do saldo
...
def salvar_dados(conta, filename="banco_dados.json"):
    ...
    with open(filename, "w") as f:  # Abre o arquivo em modo de escrita
        # Salva os dados no arquivo formatado
        json.dump(dados, f, indent=4)  # indent=4 para formatação legível
...
def carregar_dados(filename="banco_dados.json"):
    ...
    try:
        with open(filename, "r") as f:
            ...
            # Retorna uma nova instância com os dados carregados
            return ContaBancaria(dados["numero_conta"], dados["saldo"], dados["extrato"])
    except FileNotFoundError:  # Se o arquivo não existir
        print("Arquivo de dados não encontrado. Criando nova conta com número padrão '@0000-0'.")
        return ContaBancaria("00000-0")  # Retorna nova conta com número padrão

if __name__ == "__main__":
    minha_conta = carregar_dados()  # Carrega os dados da conta principal
    
    # Exemplo de como ter uma segunda conta (para transferência)
    outra_conta = carregar_dados(filename="banco_secundario.json")
    if outra_conta._numero_conta == "00000-0":  # Se for uma conta recém-criada
        outra_conta._numero_conta = "54321-V"  # Atribui um número real para demonstração
    ...
    while True:  # Loop principal do menu
        ...
        if opcao == 1:
            ...
            salvar_dados(outra_conta, filename="banco_secundario.json")  # Salvar a segunda conta
